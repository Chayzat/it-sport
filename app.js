...
_a = FifteenthTitle, _FifteenthTitle_get = function _FifteenthTitle_get() {
    ...
    const itemWithTitle = resData.filter((item) => item.id === __classPrivateFieldGet(FifteenthTitle, _a, "f", _FifteenthTitle_ID));
    const title = itemWithTitle.map((item) => item.title);
    return title;
};
